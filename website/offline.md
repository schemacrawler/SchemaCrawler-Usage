...
offline catalog snapshot, for future use, using the [`serialize`](serialize.md) command. 
...
Use the [`serialize`](serialize.md) command with `--output-format=ser` to 
...
and no [limit, filter or grep options](schemacrawler-shell.md).
...
to use. You can apply [limit, filter or grep options](schemacrawler-shell.md)
too.
...
For a hands-on walkthrough, see the [offline snapshot example](examples/offline.md).